...
var ShareInfo = require('../share_info');
...
route.get('/', function(req, res, next) {
	...
	exchange.AccessService.instance.getRatesBySource({
...
route.get('/rates/:uniquename', function(req, res, next) {
	...
	var currentCulture = res.locals.currentCulture,
		...
		source = exchange.data.getSourceByUniqueName(config.country, uniquename);
	...
	var keys = exchange.data.formatRateKeys(config.country, res.locals.currentDate, source.id, config.currencies, 0, 3);

	exchange.AccessService.instance.getFirstDateRates({
		fillCurrency: true,
		keys: keys
	}).then(function(rates) {
		res.render('rates', {
