...
module.exports = function(req, res, next) {
  ...
  Promise.props({
      sources: getSources(),
      rates: getWidgetRates()
    }).then(function(result) {
      res.locals.banks = result.sources;
      res.locals.exchangeWidget.rates = result.rates;
      if (result.rates && result.rates.length > 0) {
        res.locals.exchangeWidget.source = _.find(res.locals.mainSources, 'id', result.rates[0].source);
        res.locals.exchangeWidget.date = result.rates[0].date;
      }
    }).finally(next);
